...
def poly_decrypter(key, length, relative_address, args):
    ...
    reg5_to_0 = get_set_reg_to_0(5, registers)
    get_key_litteral_set = [
#TODO REPLACE ALL THESE LINE BY random.choice(reg0_to_0).append("MOV "+registers[0]+", "+key)  SO IT ALLOWS FOR MULTIPLE INSTRUCTION AND SET_REG_TO_0
        [random.choice(reg0_to_0)[0], "MOV "+registers[0]+", "+hex(key)],
        [random.choice(reg0_to_0)[0], "SUB "+registers[0]+", "+hex(key*-1)],
        [random.choice(reg0_to_0)[0], "ADD "+registers[0]+", "+hex(key)],
        [random.choice(reg0_to_0)[0], "ADD "+registers[0]+", "+hex(key)],
    ...
    get_len_litteral_set = [
        [random.choice(reg1_to_0)[0], "MOV "+registers[1]+", "+hex(length)],
        [random.choice(reg1_to_0)[0], "SUB "+registers[1]+", "+hex(length*-1)],
        [random.choice(reg1_to_0)[0], "ADD "+registers[1]+", "+hex(length)],
        [random.choice(reg1_to_0)[0], "ADD "+registers[1]+", "+hex(length)],
    ...
    get_relative_address_litteral_set = [
        [random.choice(reg2_to_0)[0], "MOV "+registers[2]+", "+hex(relative_address)],
        [random.choice(reg2_to_0)[0], "SUB "+registers[2]+", "+hex(relative_address*-1)],
        [random.choice(reg2_to_0)[0], "ADD "+registers[2]+", "+hex(relative_address)],
        [random.choice(reg2_to_0)[0], "ADD "+registers[2]+", "+hex(relative_address)],
    ...
    final_decoder = reg_preservation_push + get_key + get_len + get_relative_address + get_rip
